...
- **共识检测**: 
  - 优先使用embedding向量（火山引擎doubao-embedding模型）计算立场相似度
  - 通过余弦相似度（Cosine Similarity）精确测量Agent立场的一致性
  - 如果embedding API不可用，自动降级到简单文本相似度（Jaccard相似度）
  - 生成相似度矩阵，识别最不相似的Agent配对
- **顽固检测**: 
  - 计算Agent前后轮的自相似度，识别不愿改变立场的Agent
  - 连续2轮自相似度 > 0.98 时触发顽固警告
  - Host会要求顽固Agent修改假设或降低置信度
- **动态调整**: 根据讨论进展自动调整策略
  - 共识 > 0.90 → 进入收敛阶段
  - 0.70 < 共识 ≤ 0.90 → 继续讨论
  - 共识 ≤ 0.70 → 强制反方角色
...
多Agent系统使用向量相似度来量化Agent之间的立场差异：

1. **位置摘要提取**：每个Agent输出包含一个结构化的位置摘要（conclusion + key_reasons + assumptions）

2. **文本向量化**：
   - **方法A（推荐）**：使用火山引擎embedding模型将文本转换为768维向量
   - **方法B（fallback）**：使用Jaccard相似度（基于关键词重叠）

3. **相似度计算**：
   - 使用余弦相似度（Cosine Similarity）计算向量间的夹角
   - 生成N×N相似度矩阵（N为Agent数量）
...
- **精确量化**：将抽象的"立场"转换为可计算的数值
- **自动降级**：embedding不可用时自动切换到简单方法
- **实时反馈**：每轮都重新计算，动态调整策略
- **可视化**：共识趋势图直观展示讨论进展
...
// 相似度矩阵示例（3个Agent）
[
  [1.0,  0.92, 0.88],  // Agent0与各Agent的相似度
  [0.92, 1.0,  0.81],  // Agent1与各Agent的相似度
  [0.88, 0.81, 1.0 ]   // Agent2与各Agent的相似度
]

// 平均相似度 = (0.92 + 0.88 + 0.81) / 3 = 0.87 (中等共识)
```
...
### 问题2: Embedding API不可用

**现象**:
- 后端日志显示 `404: doubao-embedding does not exist`
- 出现 "⚠️ [Host] Embedding计算失败，使用简单方法" 警告

**解决方法**:
- ✅ **系统会自动降级到简单文本相似度** - 功能不受影响
- 简单相似度基于关键词重叠度（Jaccard相似度），虽然精度略低但足够使用
- 如果想使用高精度embedding相似度：
  - 检查 `ARK_EMBEDDING_API_URL` 和 `ARK_EMBEDDING_MODEL` 配置
  - 确认你的API Key有权限访问embedding模型
  - 或者联系火山引擎开通embedding模型权限
