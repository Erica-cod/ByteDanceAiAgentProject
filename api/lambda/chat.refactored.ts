/**
 * Chat API - 符合 Modern.js BFF 规范（重构版）
 * 路由: /api/chat
 * 
 * 支持流式响应 (SSE)
 * 
 * 架构优化：
 * - 类型定义 → api/types/chat.ts
 * - System Prompt → api/config/systemPrompt.ts
 * - 模型调用 → api/utils/llmCaller.ts
 * - 工具执行 → api/utils/toolExecutor.ts
 * - 内容提取 → api/utils/contentExtractor.ts
 * - SSE 处理 → api/handlers/sseHandler.ts
 * - 多Agent处理 → api/handlers/multiAgentHandler.ts
 */
...
// 导入类型定义
import type { RequestOption, ChatRequestData, tooManyRequests } from '../types/chat.js';
import { tooManyRequests as tooManyRequestsResponse } from '../types/chat.js';
...
// 导入处理器
import { streamToSSEResponse, streamVolcengineToSSEResponse } from '../handlers/sseHandler.js';
