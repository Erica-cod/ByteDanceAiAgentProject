...
describe('tool fallback chain + redis cache (optional)', () => {
  ...
  test('cache write -> circuit open -> fallback returns cache', async () => {
    ...
    // 动态导入：确保读取到最新 env
    const { toolRegistry } = await import('../../api/tools/core/registry/tool-registry.js');
    const { toolExecutor } = await import('../../api/tools/core/execution/tool-executor.js');
    const { cacheManager } = await import('../../api/tools/core/cache/cache-manager.js');
    const { circuitBreaker } = await import('../../api/tools/core/resilience/circuit-breaker.js');
